...
function PlayerSelect({
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="font-bold text-[#1e3a5f]">{selected ? selected.name : "選択してください"}</span>
...
export default function SalaryDashboard({ players, pitchers }: SalaryDashboardProps) {
  ...
  const pitcher = selected ? pitchers.find((p) => p.name === selected.name) : undefined;
  const salary = selected ? calcSalary(selected, pitcher) : 0;
  const rank = getSalaryRank(salary);
  const basePay = 10_000_000;

  // 内訳
  const hitBonus = selected ? selected.hits * 500_000 : 0;
  const hrBonus = selected ? selected.homeRuns * 1_000_000 : 0;
  const rbiBonus = selected ? selected.rbi * 300_000 : 0;
  const sbBonus = selected ? selected.stolenBases * 100_000 : 0;
  const kPenalty = selected ? selected.strikeouts * 50_000 : 0;
  const winBonus = pitcher ? pitcher.wins * 700_000 : 0;
  const kpBonus = pitcher ? pitcher.strikeouts * 100_000 : 0;

  const count = useMotionValue(0);
  const rounded = useTransform(count, Math.round);
  const formattedSalary = useTransform(rounded, (value) => `¥${value.toLocaleString()}`);
  ...
  useEffect(() => {
    if (selected) {
      ...
      const controls = animate(count, salary, {
        duration: 1.5,
        ease: "easeOut",
        onComplete: () => setIsApproved(true),
      });
      return controls.stop;
    }
  }, [selected, salary, count]);
      ...
      <AnimatePresence mode="wait">
        {!selected ? (
          ...
          >
            ...
            <p className="text-slate-400 text-xs">推定年俸を計算するには、上のドロップダウンから対象者を選んでください。</p>
          </motion.div>
        ) : (
          <motion.div
            key="salary-card"
          ...
          >
            {/* メイン給与カード */}
            <div className="bg-gradient-to-br from-[#1e3a5f] to-[#2a5298] rounded-2xl p-6 text-white relative overflow-hidden">
              ...
              <div className="relative">
                <motion.p className="text-4xl font-black tracking-tight mt-1 h-10">
                  {formattedSalary}
                </motion.p>
                {/* 契約更改完了(APPROVED)バッジ */}
                <AnimatePresence>
                  {isApproved && (
                    <motion.div
                      ...
                      className="absolute -right-2 top-0 border-2 border-red-500 text-red-500 bg-white/10 px-3 py-1 rounded text-[10px] font-black tracking-widest uppercase shadow-sm backdrop-blur-sm"
                    >
